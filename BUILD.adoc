= ANARI^(R)^ Specification Build Instructions and Notes
:toc2:
:toclevels: 2
...
This README describes how to build the ANARI API specification, reference
...
First, clone the Khronos Github repository containing the ANARI
specification to your local Linux, Windows, or Mac PC.
The repository is located at https://github.com/KhronosGroup/ANARI-Docs/.
...
where <username> and <uid> are your username and UID respectively, and
<vulkan-path> is the path to the cloned repository.
This runs the image with the cloned repository under /vulkan and accesses it
as the specified user (which should be your own username and UID), so that
it doesn't get filled with files owned by another user.
...
which builds an HTML5 specification output for the core ANARI 1.2
...
====
  * `make all` takes a long time to run, and generates outputs that are
    ...
    `make html` just generates the HTML target, which is often all that's
    needed for spec bugfixes not involving extensions.
  * The default `make` options build a ANARI 1.2 specification with no
    optional extensions.
...
"`styleguide`" (ANARI^(R)^ Documentation and Extensions: Procedures and Conventions)::
...
or more specified targets for specified extensions:
  ...
  * `makeAllExts` -- generate outputs with all ANARI extensions enabled.
...
guide`"] (formally titled "`ANARI Documentation and Extensions: Procedures
...
This section describes the software components used by the ANARI spec
toolchain.
...
Once docker is installed and the vulkan-docs image is available, it can be
...
link:https://github.com/KhronosGroup/DockerContainers/blob/master/Dockerfile.vulkan-docs[Dockerfile.vulkan.docs]
used to build our Docker image.

It should be possible to apply the same steps in a Windows Subsystem for
Linux (WSL) environment on Windows 10, as well.
...
link:https://github.com/KhronosGroup/ANARI-Docs/blob/v1.2.135/BUILD.adoc#depends[the
...
we cannot support every possible environment.
The Docker image is a straightforward way to use the ANARI-Docs repository
