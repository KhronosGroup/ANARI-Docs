...
import anapi as api
from anariconventions import AnariConventions as APIConventions
...
def printCopyrightSourceComments(fp):
    ...
    file."""
    print('// Copyright (c) 2014-2021 Khronos Group. This work is licensed under a', file=fp)
...
def xrefRewriteInitialize():
    ...
    # These are xrefs to ANARI API entities, rewritten to link to refpages
...
def xrefRewrite(text, specURL):
    ...
    Xrefs which are to ANARI refpages are rewritten to link to those
        ...
        for refPage in keys:
            ...
            if refPage not in api.alias:
                ...
                if 'FlagBits' in refPage and conventions.unified_flag_refpages:
                    ...
                    # Previously the ANARI script would only emit non-empty
