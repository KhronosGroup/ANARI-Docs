...
def releaseNum():
    """Return the ANARI release number, used for tags."""
    return '$REVISION'
...
def buildRelease(label,
    ...
    print('(cd ', outdir, '&& rm -rf',
          'html chunked pdf',
          'man config checks',
          'anspec.html styleguide.html registry.html',
          ')')
...
def buildBranch(targetDir,
    ...
    # GLSL repository and are redirected from the ANARI registy website.
...
def createTags(releaseNum, tagdate):
    ...
    print('echo git tag -a -m \\"Tag ANARI API specification for 1.2.' +
