...
if __name__ == '__main__':
    ...
    # For each alias in the anapi alias map, create a symlink if it
    # doesn't exist - and warn if it does exist.

    for key in anapi.alias:
        alias = key + '.html'
        src = anapi.alias[key] + '.html'

        if not os.access(src, os.R_OK):
            # This shouldn't happen, but is possible if anapi.py isn't
